...
## Who do you think is happier Europeans or Americans? Hard to tell isn't it? Let's do some calculations

```{r}
#assign name to dataframe

happiness_stats<- whr_19 %>% 
  
#remove NAs
  
na.omit(happiness_stats) %>% 
...
```{r}
ggplot(happiness_stats,
       
       aes(x=reorder(continent, mean_score), 
           y=mean_score, 
           colour=continent)) +
  ...
  labs(x="Continent
       ...
       y= "
       Happiness Score",
       title= "Difference in Happiness between Americas and Europe:
Looking at confidence intervals
", 
       subtitle = "The CI's of Europe and the Americas overlap")+ 
